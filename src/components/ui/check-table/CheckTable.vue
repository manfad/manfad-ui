...
const decoratedRows = computed(() =>
  props.rows.map((row, index) => {
    ...
    return { row, key, selected: selected.value.includes(key) }
  }),
)

const allSelected = computed(() =>
  decoratedRows.value.length > 0 && decoratedRows.value.every(entry => entry.selected),
)

const indeterminate = computed(() =>
  !allSelected.value && decoratedRows.value.some(entry => entry.selected),
)
...
function toggleAll(value: boolean | undefined): void {
  selected.value = value ? decoratedRows.value.map(entry => entry.key) : []
}
...
const LEADING_WIDTH = 48 // checkbox control column (w-12)
...
const columnLayout = computed(() => {
  ...
  const leftCols = leftKeys
    ...
    .filter((col): col is CheckTableColumn => col != null)
...
function pinStyle(col: CheckTableColumn): Record<string, string> | undefined {
...
function pinClass(col: CheckTableColumn, isHeader: boolean): string | undefined {
  ...
  <Table v-bind="$attrs" :class="props.class">
    ...
    <TableBody>
      <TableRow
        ...
        :key="entry.key"
        class="cursor-pointer data-[state=selected]:bg-muted/50"
        :data-state="entry.selected ? 'selected' : undefined"
        @click="onRowClick($event, entry.key)"
      >
        <TableCell
          :class="cn('w-12', hasLeftPins && 'sticky left-0 z-10 bg-background data-[state=selected]:bg-muted')"
          :data-state="entry.selected && hasLeftPins ? 'selected' : undefined"
        >
          <Checkbox
        ...
        <TableCell
          v-for="col in columnLayout.ordered"
          :key="col.key"
          :class="cn(col.class, pinClass(col, false), isPinned(col) && 'data-[state=selected]:bg-muted')"
          :style="pinStyle(col)"
          :data-state="entry.selected && isPinned(col) ? 'selected' : undefined"
        >
          <slot
            :name="`cell-${col.key}`"
            :row="entry.row"
